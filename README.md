...
- 預設採用 **Clean Emerald 淺色現代主題**，搭配內政部國土測繪中心**「臺灣通用電子地圖 (NLSC)」**，具備最詳盡的國道路網、交流道出入口標記與等高地形，完全免 API Key、零浮水印且極速載入。
- 地圖右上角提供**圖層切換選單 (Layer Switcher)**，支援一鍵切換「臺灣通用電子地圖」、「ESRI 極簡淺灰底圖」與「OpenStreetMap」。
- 頂部導覽列提供**深/淺模式一鍵切換按鈕**（🌙 / ☀️），並以 `localStorage` 記憶偏好；切換至深色時自動載入無浮水印之「ESRI 深黑極簡底圖」。
